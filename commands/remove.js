...
const { TrackUtils } = require("erela.js");
  ...
  module.exports = {
    ...
    aliases: ["rm", "tira", "remover", "remove"],
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    let rm = new MessageEmbed()
      .setDescription(`✅ **|** Faixa **\`${Number(args[0])}\`** removida da fila!`)
      .setColor("GREEN")
      if (isNaN(args[0]))rm.setDescription(`**Use - **${client.config.prefix}\`remover [track]\``);
      if (args[0] > player.queue.length)
      rm.setDescription(`A fila tem apenas ${player.queue.length} músicas!`);
   ...
   */
    ...
    let rm = new MessageEmbed()
      ...
      if (isNaN(args[0]))rm.setDescription(`Use: ${client.config.prefix}\`remover [track]\``);
      if (args[0] > player.queue.length)
      rm.setDescription(`A fila tem apenas ${player.queue.length}!`);
